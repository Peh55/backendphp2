...
class M_aluno extends CI_Model {
    public function inserirAluno($ra, $idCurso, $nome, $estatus){
        ...
        //realizar a instãncia do objeto curso
        $curso = new M_Curso();
    ...
    public function consultarSoAluno($ra){
        //----------------------------------------------------
        //Função que servirá somente para veficar se o curso está na base de dados
    ...
    public function apagarAluno($ra){
        $retornoAluno = $this->consultarAluno($ra);
        if ($retornoAluno['codigo'] == 1){
    ...
    public function reativarAluno($ra, $idCurso, $nome){
        $curso = new M_Curso();
        $retorno = $this->consultarSoAluno($ra);
        $retornoCurso = $curso->consultarSoCurso($idCurso);
        ...
        if($retornoAluno['codigo'] == 1){
            ...
            if ($retornoAluno['codigo'] == 2){
                ...
                $sql = "insert into aluno(ra)
                values('$ra'); select last_insert_ra()";
